...
namespace OpenLibraryApi
{
    public class BooksSearchExecutor 
    {
        ...
        public async Task<string> ExecuteSearch(BooksSearchType booksSearchType, string searchParams)
        {
            IBooksSearch booksSearch = GetSearchBooksObject(booksSearchType);
            var searchUrl = booksSearch.CreateSearchUrl(searchParams);

             return await _apiConnection.Connect(searchUrl); //ciekawy przypadek bez await - ogarnąć to
